...
- [x] UniMate MARA registration acknowledgement displayed in page footer with live link to MARA public register *(Footer consumes `brand.mara_number` + `https://www.mara.gov.au/search-the-register-of-migration-agents/`; specific MARN string deferred to client until UniMate provides — pre-meeting fix 2026-04-25 swapped literal `[PENDING_FROM_UNIMATE]` strings for honest "MARA-registered (UniMate Pty Ltd)" copy across all user-visible surfaces)*
