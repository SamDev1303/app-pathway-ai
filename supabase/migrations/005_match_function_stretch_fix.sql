-- Atlas AI — P4 UniMatch engine RPC (stretch_flag fix)
-- Region: ap-southeast-1 (Singapore)
-- Apply via: supabase db push (applied AFTER 004_match_function.sql)
--
-- Scope of change vs 004:
--   Extend stretch_flag predicate to also fire when budget_score = 0
--   (far-stretch budget band, 20–50% over student's stated budget).
--
--   BEFORE (004, line ~162–164):
...
--     (budget_score < 0 OR ielts_score < 0) = false
--     AND (
--       budget_score = v_w_budget / 2           -- mild budget stretch (0–20% over)
--       OR budget_score = 0                     -- far budget stretch (20–50% over)
--       OR ielts_score = v_w_ielts / 2          -- IELTS half-band stretch
--     )
--     AS stretch_flag
--
-- Why: RESEARCH.md §Stretch Math promises "far-stretch courses should surface in
-- stretch tier, not be invisible". In 004, a course whose ONLY penalty was a
-- budget_score of 0 (20–50% over budget) would land in strong with zero budget
-- contribution — invisible far-stretch. This fix routes them to the stretch tier
-- where the budget_verdict='far_stretch' reason is already rendered correctly.
--
-- Nothing else changes: weights are unchanged, hard_cut rules unchanged, ranking
-- tiebreakers unchanged, side-effect UPDATE unchanged, grants unchanged.
...
BEGIN
  ...
  course_totals AS (
    SELECT *,
      ...
      -- stretch_flag: 50%-of-weight on budget/ielts OR far-stretch budget band (budget_score=0)
      -- (fix vs 004: previously only (budget_score = v_w_budget / 2 OR ielts_score = v_w_ielts / 2);
      --  added `OR budget_score = 0` so 20–50%-over-budget courses surface in stretch tier
      --  instead of contaminating strong. See 005 header for rationale.)
      (budget_score < 0 OR ielts_score < 0) = false
      AND (
        budget_score = v_w_budget / 2
        OR budget_score = 0
        OR ielts_score = v_w_ielts / 2
      )
      AS stretch_flag
    FROM course_scores
  ),
        ...
        'stretch_reason',
          CASE
            WHEN r.budget_score = v_w_budget / 2 AND v_lead.tuition_budget_aud IS NOT NULL THEN
              ...
              '%.'
            WHEN r.budget_score = 0 AND v_lead.tuition_budget_aud IS NOT NULL THEN
              'Tuition above your stated budget by roughly ' ||
              ROUND(100.0 * (r.indicative_fee - v_lead.tuition_budget_aud) / v_lead.tuition_budget_aud)::text ||
              '%.'
...
-- (Re-applied here so a CREATE OR REPLACE that somehow reset grants is re-locked.)
-- ================================================================
