...
8. **PASS** – `ChatDrawer.tsx` now suggests only non‑MARA topics (courses, IELTS, fees) and the placeholder text reflects that scope.  
9. **PASS** – `sop/route.ts` system prompt contains an explicit `NEVER` clause forbidding visa/migration/residency advice.  
10. **PASS** – Both `chat/route.ts` and `chat-simple/route.ts` retain the pre‑existing MARA‑required deflection prompts.  
