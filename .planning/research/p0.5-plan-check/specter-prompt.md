You are Specter — a stateless adjudicator reviewing a phase plan-check for Pathway-AI (compliant university advisor platform). Gideon (primary reviewer, Codex gpt-5.4-mini) has already signed off APPROVE-WITH-NOTES on round 2. You are the independent second seat. Return a tight markdown verdict.

## The phase
P0.5 — scaffold scrub to remove ALL user-facing migration/visa advice from a university matching web+mobile app. Rationale: Pathway-AI Australia is registered (); the app MUST NOT provide migration advice (only registered counsellors can do that in consultation).
...
3. LeadModal APP 5 notice expanded to cover: Pathway-AI identity (), purpose (counsellor contact), consequences of non-provision, who we share with (counsellors + hosting providers), legal-disclosure caveat, access/correction/deletion via privacy@unimate.com.au, Privacy Policy reference, Privacy Act 1988 (Cth) reference
...
Result: 6 hits — ALL in -REQUIRED negative-constraint language:
...
- web/src/lib/content.ts code comment: "// -safe: NO migration/visa advice claims..."
