...
| # | Must-have | Status |
|---|-----------|--------|
| 1 | Site-wide disclaimer footer | ✅ Wave 4 — layout.tsx owns `<Footer />` |
| 2 | registration # + link in footer | ✅ Wave 1 — placeholder + CI gate mechanism |
...
| 9 | CI grep gate | ✅ Wave 5b — 3-job workflow (local-pending push) |

**Mid-session scope expansions (authorized by Sam):**

- **Wave 0** — scrub fake `` + `QEAC P538` + `ABN 12 345 678 901` across 7 files (Playwright-verified fake via portal.mara.gov.au)
...
```
b24717c feat(phase-4.5): wave 0 — scrub fake /QEAC/ABN identifiers
...
1. ** registration number () + QEAC + ABN replacement** — CI gate enforcement deadline **2026-04-27 23:59 AEST**
...
- ✅ -safe deflection pattern defined
